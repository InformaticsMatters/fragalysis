...
Run like this:
python -m frag.network.scripts.standardise_chemspace_bb_compounds_3 -i ~/data/structures/chemspace/Jul2019_CS_BB_MOD_selected_set_SMILES.txt.gz -s 2
...
def standardise_vendor_compounds(file_name, source_id, limit):
    ...
    :param source_id: The ID of the source of these molecules
    ...
    with gzip.open(file_name, 'rt') as gzip_file:
        ...
        for line in gzip_file:
            ...
            price_s = fields[price_col].strip()
            prices= None
            if price_s:
                prices = {1000: int(price_s)}
